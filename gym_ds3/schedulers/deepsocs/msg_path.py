...
from gym_ds3.envs.utils.helper_deepsocs import SparseMat, absorb_sp_mats
...
class Postman(object):
    ...
    def get_msg_path(self, job_dags):
        ...
        if job_dags_changed:
            self.msg_mats, self.msg_masks = get_msg_path(self.args, job_dags)
...
def get_msg_path(args, job_dags):
    """
    ...
    for job_dag in job_dags:
        msg_mat, msg_mask = get_bottom_up_paths(args, job_dag)
    ...
    if len(job_dags) > 0:
        ...
        msg_masks = merge_masks(args, msg_masks)
...
def get_bottom_up_paths(args, job_dag):
    """
    ...
    for depth in range(args.max_depth):
        ...
        for n in new_frontier:
            for child in n.child_nodes:
                sp_mat.add(row=n.base_ID, col=child.base_ID, data=1)
            msg_masks[depth, n.base_ID] = 1
...
def get_dag_summ_backward_map(job_dags):
    ...
    for job_dag in job_dags:
        for node in job_dag.nodes:
            dag_summ_backward_map[base + node.base_ID, j_idx] = 1
...
def get_unfinished_nodes_summ_mat(job_dags):
    ...
    for job_dag in job_dags:
        ...
        for node in job_dag.nodes:
            if node.finish_time == np.inf:
                summ_row_idx.append(j_idx)
                summ_col_idx.append(base + node.base_ID)
                summ_data.append(1)
    ...
    summerize_mat = tf.compat.v1.SparseTensorValue(
